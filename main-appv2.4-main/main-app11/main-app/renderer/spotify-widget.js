        ...
        // Skeleton "loadup" shown while authenticated and waiting for the first track
        // payload. Swaps the idle/active states out for a shimmering mirror of the
        // player so the connect → playing transition never shows an empty/idle flash.
        ...
        function updateSpotifyUI(data) {
            ...
            // Any authoritative payload (connected or not) means the loadup is done.
            ...
            // Anchor for smooth between-poll progress interpolation (see
            // tickSpotifyProgress): remember the authoritative position and the wall
            // clock at this poll so the ticker can advance it fluidly until the next.
            spotifyProgressAnchor = { positionMs: data.track.progress_ms || 0, at: Date.now() };
        ...
        // Spotify is polled every 2.5s (its rate limit). Setting the bar only on each
        // poll makes it visibly step every 2.5s. Instead we anchor the authoritative
        // position at each poll (spotifyProgressAnchor) and, 4×/second, advance the
        // displayed position by real elapsed time while playing — so the bar and the
        // current-time label glide continuously and stay accurate, resyncing on every
        // poll and on any user action (play/pause/seek/skip).
        let spotifyProgressAnchor = { positionMs: 0, at: 0 };
        let spotifyProgressTickHandle = null;

        function tickSpotifyProgress() {
            // spotifyCurrentTrack / spotifyIsPlaying / isDraggingProgress are lexical
            // globals declared in beat-glow.js (loaded first), so reference them
            // directly — they are NOT window properties.
            if (isDraggingProgress) return; // don't fight a manual scrub
            const track = spotifyCurrentTrack;
            if (!track || !track.duration_ms) return;
            let pos = spotifyProgressAnchor.positionMs || 0;
            if (spotifyIsPlaying) pos += Date.now() - spotifyProgressAnchor.at;
            pos = Math.max(0, Math.min(track.duration_ms, pos));

            const bar = document.getElementById('spotify-progress-bar');
            if (bar) bar.style.width = ((pos / track.duration_ms) * 100) + '%';
            const cur = document.getElementById('spotify-time-current');
            if (cur) cur.textContent = formatMsTime(pos);
            const tot = document.getElementById('spotify-time-total');
            if (tot && (localStorage.getItem('spotifyTimerMode') || 'total') === 'remaining') {
                tot.textContent = formatMsTime(Math.max(0, track.duration_ms - pos));
            }
        }

        function startSpotifyProgressTicker() {
            if (!spotifyProgressTickHandle) spotifyProgressTickHandle = setInterval(tickSpotifyProgress, 250);
        }
        function stopSpotifyProgressTicker() {
            if (spotifyProgressTickHandle) { clearInterval(spotifyProgressTickHandle); spotifyProgressTickHandle = null; }
        }

        function startSpotifyPolling() {
            stopSpotifyPolling();
            // Paint the skeleton loadup right away — startSpotifyPolling is only ever
            // reached once authenticated (initSpotifyWidget / connectSpotify), so the
            // user is connecting, not idle. It's swapped out by updateSpotifyUI the
            // moment the first track payload arrives.
            showSpotifyLoading();
